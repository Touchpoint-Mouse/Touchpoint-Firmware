...
#include "SROM.h"
#include <Mouse.h>  // RP2040 built-in USB HID Mouse support
...
void sendMouseReport(int deltaX, int deltaY) {
  // Convert to signed 8-bit values for Mouse.move()
  // Mouse.move() takes signed char (int8_t) values
  int8_t moveX = constrain(deltaX, -127, 127);
  int8_t moveY = constrain(deltaY, -127, 127);
  
  Mouse.move(moveX, moveY, 0);  // x, y, wheel
}
...
void loop() {
  ...
  if(currentTime - lastCheck >= 10) {  // Check every 10ms
    ...
    // Send mouse movement if motion detected
    if (hasMotion && onSurface) {
      int16_t x = convTwosComp(deltaX);
      int16_t y = convTwosComp(deltaY);
      sendMouseReport(x, y);
    }
  }
}
